...
?>
<div class="wrap">
	<h1><?php _e( 'Hide Admin Menu', 'cnw-hide-admin-menu' ); ?></h1>
	<div class="notice notice-warning">
		<p><?php esc_html_e( 'Checking a checkbox disables the access to the menu item for the corresponding role.', 'cnw-hide-admin-menu' ); ?></p>
		<p><?php esc_html_e( 'If no checkbox is available, it means the corresponding role cannot access the menu item by default (and you can\'t enable it with this plugin either).', 'cnw-hide-admin-menu' ); ?></p>
		<?php if ( is_multisite() ) : ?>
			<p><?php esc_html_e( 'Disabled (grey) checkbox means the corresponding menu is hidden network-wide.', 'cnw-hide-admin-menu' ); ?></p>
		<?php endif; ?>
		<p><strong><?php esc_html_e( 'If you\'re the Administrator, don\'t check any checkboxes unless you really want to lock yourself out of that menu item.', 'cnw-hide-admin-menu' ); ?></strong></p>
	</div>
	...
	<form method="post" action="<?php echo $action_page; ?>">
		...
		<table class="widefat" id="ham-selection">
			<thead>
			<tr>
				<th><?php _e( 'Menu Item', 'cnw-hide-admin-menu' ); ?></th>
				<?php
			...
			foreach ( $ham_menu as $top_item ) {
				...
				if ( $top_text == '' ) {
					...
					if ( in_array( 'wp-menu-separator', $top_item ) || in_array( 'wp-menu-separator-last', $top_item ) ) {
						$top_text = __( 'Separator', 'cnw-hide-admin-menu' );
					}
		...
		<p class="submit">
			<?php submit_button( __( 'Save Settings', 'cnw-hide-admin-menu' ), 'primary', 'submit', false ); ?>
			<?php submit_button( __( 'Reset', 'cnw-hide-admin-menu' ), 'primary', 'reset', false ); ?>
		</p>
	</form>
</div>
