...
?>
	...
	<div class="wrap">
		<h2><?php _e( 'Hide Admin Bar', 'cnw-hide-admin-menu' ); ?></h2>
...
function ham_show_admin_bar_item( $node, $depth = 0 ) {
	...
	if ( ! $node->group && ! in_array( $node->id, $ham_displayed ) ) {
		...
		switch ( $node->id ) {
			case 'wp-logo':
				$title = __( 'WordPress Logo', 'cnw-hide-admin-menu' );
				break;
			case 'comments':
				$title = __( 'Comments', 'cnw-hide-admin-menu' );
				break;
			case 'updates':
				$title = __( 'Updates', 'cnw-hide-admin-menu' );
				break;
			case 'my-account':
				$title = __( 'My Account', 'cnw-hide-admin-menu' );
				break;
			case 'user-info':
				$title = __( 'User Info', 'cnw-hide-admin-menu' );
				break;
			default:
