...
class HAM_Imex extends Peace_Admin_Page {
	...
	public function __construct() {
		$page_options = array(
			'menu_title' => __( 'Import & Export', 'cnw-hide-admin-menu' ),
	...
	public function import() {
		...
		if ( ! isset( $_FILES['setting']['error'] ) || $_FILES['setting']['error'] != UPLOAD_ERR_OK ) {
			$error = __( 'Error uploading file. Please try again.', 'cnw-hide-admin-menu' );
		} else {
			$content = file_get_contents( $_FILES['setting']['tmp_name'] );

			// No file content or error reading
			if ( ! $content ) {
				$error = __( 'Cannot read file content. Please try again.', 'cnw-hide-admin-menu' );
			}
			...
			if ( false === $menu_option || false === $bar_option ) {
				$error = __( 'Invalid file content. Please try again.', 'cnw-hide-admin-menu' );
			}
		}
		...
		if ( $error ) {
			...
			add_settings_error( $this->page_id, '', __( 'Settings imported successfully.', 'cnw-hide-admin-menu' ), 'updated' );
		}
	}
