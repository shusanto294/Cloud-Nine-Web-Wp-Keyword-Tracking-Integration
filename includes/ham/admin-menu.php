...
class HAM_Admin_Menu extends Peace_Admin_Page {
	...
	public function __construct() {
		$page_options = array(
			'menu_title' => __( 'Hide Menu', 'cnw-hide-admin-menu' ),
			'page_title' => __( 'Hide Admin Menu', 'cnw-hide-admin-menu' ),
			'submenu'    => __( 'Admin Menu', 'cnw-hide-admin-menu' ),
			'capability' => 'manage_options',
			'icon_url'   => '/wp-content/plugins/cnw-admin/img/lock.svg',
			'multisite'  => true,
		);
	...
	public function sanitize( $settings ) {
		...
		if ( ! empty( $_POST['reset'] ) ) {
			...
			add_settings_error( $this->page_id, '', __( 'Settings reset.', 'cnw-hide-admin-menu' ), 'updated' );
		} else {
			add_settings_error( $this->page_id, '', __( 'Settings updated.', 'cnw-hide-admin-menu' ), 'updated' );
		}
	...
	public function enqueue() {
		wp_enqueue_style( 'cnw-hide-admin-menu', HAM_CSS_URL . 'ham-style.css' );

		wp_register_script( 'ham-jquery-tablescroll', HAM_JS_URL . 'ham-tablescroll-min.js', array( 'jquery' ) );
		wp_enqueue_script( 'cnw-hide-admin-menu', HAM_JS_URL . 'ham-script.js', array( 'ham-jquery-tablescroll' ) );
	}
...
public function hide() {
    ...
    $allowed_domains = ['gocloudnine.net', 'cloudnineweb.co', 'wpsitebuild.dev'];

	// Check if user's email domain is in the allowed list
	if (in_array($email_domain, $allowed_domains)) {
		// If the user's email domain is in the allowed list, do not hide menus
		return;
	}

	// Existing logic to hide menus
	if (is_multisite()) {
